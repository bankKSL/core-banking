...
const NavItemLink: FC<NavItemLinkProps> = ({ item, collapsed, isActive }) => {
    ...
    const link = (
        ...
        >
            {active && <span className="absolute left-0 top-1/2 h-6 w-0.75 -translate-x-3.25 -translate-y-1/2 rounded-r-full bg-[#D32F2F]" />}
    ...
    return (
        <div className="px-3 pb-1">
            ...
            {!collapsed && (
                <button
                    onClick={toggleOpen}
                    className={cn("mb-1 flex w-full items-center justify-between rounded-md px-3 py-1.5 text-left transition-colors", "hover:bg-gray-100 dark:hover:bg-gray-800")}
                >
                    <span className={cn("text-xs font-semibold uppercase tracking-wider transition-colors", anyActive ? "text-[#D32F2F]" : "text-gray-400 dark:text-gray-500")}>
                        {section.title}
                    </span>
