...
const FixedDepositDetailPage: React.FC = () => {
  ...
  const runCommand = useCallback(async (cmd: string, extra: Record<string, unknown> = {}) => {
    if (!fd) return;
  ...
  const handleClose = useCallback(async () => {
    if (!fd) return;
    const payload: Record<string, unknown> = { closedOnDate: closeDate, onAccountClosureId: Number(onAccountClosureId) };
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2 flex-wrap">
            {statusConfig && (
              <Badge variant={statusConfig.variant === "success" ? "success" : statusConfig.variant === "error" ? "error" : "info"}>
                {statusConfig.label}
              </Badge>
            )}
            {isPending && (
              <>
                <Button variant="outline" size="sm" onClick={handleApprove} disabled={actionLoading} className="text-emerald-600 border-emerald-200">
                  {actionLoading && <Loader2 className="mr-1 h-4 w-4 animate-spin" />}
                  <CheckCircle2 className="mr-1 h-4 w-4" />
                  Approve
                </Button>
                <Button variant="outline" size="sm" onClick={() => rejectFixedDeposit(fd.id).then(() => refetch())} disabled={actionLoading} className="text-red-600">
                  <XCircle className="mr-1 h-4 w-4" />
                  Reject
                </Button>
                <Button variant="outline" size="sm" onClick={() => withdrawFixedDeposit(fd.id).then(() => refetch())} disabled={actionLoading} className="text-amber-600">
            ...
            {isApproved && (
              <>
                <Button variant="outline" size="sm" onClick={handleActivate} disabled={actionLoading} className="text-emerald-600">
                  <CheckCircle2 className="mr-1 h-4 w-4" />
                  Activate
                </Button>
                <Button variant="outline" size="sm" onClick={() => undoApprovalFixedDeposit(fd.id).then(() => refetch())} disabled={actionLoading}>
            ...
            {isActive && (
              <>
                ...
                <Button variant="outline" size="sm" onClick={() => { setCloseDialogOpen(true); setOnAccountClosureId("200"); }} className="text-amber-600">
                  <XCircle className="mr-1 h-4 w-4" />
                  Premature Close
                </Button>
                <Button variant="outline" size="sm" onClick={() => { setCloseDialogOpen(true); setOnAccountClosureId("100"); }} className="text-gray-600">
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <Card>
              ...
              <CardContent className="divide-y divide-gray-100 dark:divide-gray-800">
                <InfoRow icon={<Hash className="h-4 w-4" />} label="Account No" value={<code className="text-xs">{fd.accountNo}</code>} />
                ...
                <InfoRow icon={<DollarSign className="h-4 w-4" />} label="Deposit Amount" value={formatCurrency(fd.depositAmount, fd.currency?.code)} />
                <InfoRow icon={<DollarSign className="h-4 w-4 text-emerald-500" />} label="Maturity Amount" value={formatCurrency(fd.maturityAmount ?? 0, fd.currency?.code)} />
                <InfoRow icon={<Percent className="h-4 w-4" />} label="Interest Rate" value={`${fd.nominalAnnualInterestRate ?? fd.interestRate ?? 0}%`} />
                <InfoRow icon={<Clock className="h-4 w-4" />} label="Period" value={`${fd.depositPeriod ?? "—"} ${fd.depositPeriodFrequencyType?.value?.toLowerCase() ?? ""}`} />
                <InfoRow icon={<DollarSign className="h-4 w-4" />} label="Balance" value={formatCurrency(fd.accountBalance ?? 0, fd.currency?.code)} />
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="divide-y divide-gray-100 dark:divide-gray-800">
                <InfoRow icon={<Calendar className="h-4 w-4" />} label="Submitted" value={formatDate(fd.timeline?.submittedOnDate)} />
                <InfoRow icon={<Calendar className="h-4 w-4" />} label="Approved" value={formatDate(fd.timeline?.approvedOnDate)} />
                <InfoRow icon={<Calendar className="h-4 w-4" />} label="Activated" value={formatDate(fd.timeline?.activatedOnDate)} />
                <InfoRow icon={<Calendar className="h-4 w-4" />} label="Matured" value={formatDate(fd.timeline?.maturedOnDate)} />
                <InfoRow icon={<Calendar className="h-4 w-4" />} label="Closed" value={formatDate(fd.timeline?.closedOnDate)} />
                <InfoRow icon={<DollarSign className="h-4 w-4" />} label="Maturity Date" value={formatDate(fd.maturityDate)} />
              </CardContent>
            </Card>
            ...
            <Card className="col-span-full">
              ...
              <CardContent className="grid grid-cols-2 gap-4 text-sm">
                <div><span className="text-gray-500">Compounding:</span> <span className="font-medium">{fd.interestCompoundingPeriodType?.value ?? "—"}</span></div>
                <div><span className="text-gray-500">Posting:</span> <span className="font-medium">{fd.interestPostingPeriodType?.value ?? "—"}</span></div>
                <div><span className="text-gray-500">Calculation:</span> <span className="font-medium">{fd.interestCalculationType?.value ?? "—"}</span></div>
                <div><span className="text-gray-500">Days/Year:</span> <span className="font-medium">{fd.interestCalculationDaysInYearType?.value ?? "—"}</span></div>
              </CardContent>
            </Card>
          </div>
        </TabsContent>
          ...
          <div className="space-y-4">
            ...
            {onAccountClosureId === "200" && (
              <div className="flex flex-col gap-1.5">
                ...
                <Input id="toSavingsAccountId" type="number" value={toSavingsAccountId} onChange={(e) => setToSavingsAccountId(e.target.value)} placeholder="Savings account ID" />
              </div>
            )}
            <div className="flex gap-2">
              ...
              <Button onClick={onAccountClosureId === "200" ? handlePrematureClose : handleClose} disabled={actionLoading} variant="destructive">
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="flex flex-col gap-1.5">
              ...
              <Input id="depositAmount" type="number" value={depositAmount} onChange={(e) => setDepositAmount(e.target.value)} />
            </div>
            <div className="flex flex-col gap-1.5">
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="flex flex-col gap-1.5">
              ...
              <Input id="withdrawalAmount" type="number" value={withdrawalAmount} onChange={(e) => setWithdrawalAmount(e.target.value)} />
            </div>
            <div className="flex flex-col gap-1.5">
