...
const StandingInstructionFormPage: React.FC = () => {
    ...
    const mutation = useMutation({
        mutationFn: async () => {
            if (!from.officeId || !from.clientId || !from.accountType || !from.accountId ||
                !to.officeId || !to.clientId || !to.accountType || !to.accountId ||
                !name || !amount || !validFrom) {
    ...
    const isValid =
        from.officeId && from.clientId && from.accountType && from.accountId &&
        to.officeId && to.clientId && to.accountType && to.accountId &&
        name && amount !== "" && validFrom;
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <Select
                                    ...
                                    onValueChange={(v) => setTransferType(Number(v))}
                                >
                                    <SelectTrigger>
                                        <SelectValue />
                                    </SelectTrigger>
                            ...
                            <div>
                                ...
                                <Select
                                    value={String(instructionType)}
                                    onValueChange={(v) => setInstructionType(Number(v))}
                                >
                                    <SelectTrigger>
                                        <SelectValue />
                                    </SelectTrigger>
                        ...
                        <div className="flex justify-end gap-3 pt-4">
                            <Button
                                type="button"
                                variant="outline"
                                onClick={() => navigate("/transfers/standing-instructions")}
                            >
                                Cancel
                            </Button>
                            <Button type="submit" disabled={!isValid || mutation.isPending}>
                                {mutation.isPending ? (
                                    ...
                                    <>
                                        ...
                                        {isEdit ? "Update Instruction" : "Create Instruction"}
                                    </>
                                )}
                            </Button>
                        </div>
