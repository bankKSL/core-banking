...
const DepositAccountsPage: React.FC = () => {
  ...
  const { data: accountsData, isLoading, isError, error } = useSavingsAccounts({
    limit: PAGE_SIZE,
  ...
  const columns: ColumnDef<SavingsAccount>[] = [
    ...
    { key: "accountBalance", header: "Balance", cell: (r) => <span className="font-mono text-sm font-semibold">{formatCurrency(r.accountBalance ?? 0)}</span> },
    { key: "nominalAnnualInterestRate", header: "Rate", cell: (r) => `${r.nominalAnnualInterestRate ?? 0}%` },
    { key: "status", header: "Status", cell: (r) => { const c = SAVINGS_STATUS_CONFIG[r.status?.code ?? ""]; return <StatusBadge status={c?.variant ?? "default"} size="sm">{c?.label ?? r.status?.code ?? "Unknown"}</StatusBadge>; } },
    { key: "savingsOfficerName", header: "Officer", cell: (r) => r.savingsOfficerName ?? "—" },
    {
      key: "actions", header: "",
      cell: (r) => (<Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => navigate(`/deposits/transactions?account=${r.id}`)}><Eye className="h-4 w-4" /></Button>),
    },
  ];

  if (isError) return (
    <div className="flex items-center justify-center h-64"><div className="text-center"><AlertTriangle className="mx-auto h-8 w-8 text-red-500 mb-2" /><p className="text-red-600">Failed to load deposit accounts: {String(error)}</p><Button variant="outline" className="mt-2" onClick={() => window.location.reload()}>Retry</Button></div></div>
  );
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="relative w-80"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" /><Input placeholder="Search by customer or account..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10" /></div>
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              <SelectTrigger className="w-36"><SelectValue placeholder="Status" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">All</SelectItem>
                {Object.entries(SAVINGS_STATUS_CONFIG).map(([code, cfg]) => (<SelectItem key={code} value={code}>{cfg.label}</SelectItem>))}
              </SelectContent>
            </Select>
          </div>
        </CardHeader>
