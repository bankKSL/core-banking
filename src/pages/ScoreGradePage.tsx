      ...
      <CardContent>
        <div className="flex flex-col sm:flex-row items-center gap-6">
          <div className="flex flex-col items-center gap-2">
            <div
              className={`flex h-24 w-24 items-center justify-center rounded-full ${grade.bgColor} border-4 border-current ${grade.color}`}
            >
              <span className={`text-4xl font-extrabold tracking-tight ${grade.color}`}>{grade.letter}</span>
            </div>
            <Badge className={`text-sm px-3 py-1 font-semibold ${grade.bgColor} ${grade.color} border-0`}>
              {grade.label}
            </Badge>
          </div>
