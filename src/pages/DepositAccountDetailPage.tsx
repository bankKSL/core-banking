...
const formatCurrency = (n?: number, code = "USD") => (n != null ? new Intl.NumberFormat("en-US", { style: "currency", currency: code, maximumFractionDigits: 2 }).format(n) : "—");
...
const DepositAccountDetailPage: React.FC = () => {
    ...
    return (
        <div className="p-6 max-w-4xl m-auto space-y-6">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {isActive && (
                            <>
                                ...
                                <Button variant="outline" size="sm" onClick={() => handleCommand("close")} disabled={acting} className="text-gray-600">
                                    <XCircle className="mr-1 h-4 w-4" />
                                    Close
                                </Button>
                            </>
                    ...
                    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                        <Card>
                            ...
                            <CardContent className="divide-y divide-gray-100 dark:divide-gray-800">
                                <InfoRow icon={<Hash className="h-4 w-4" />} label="Account No" value={<code className="text-xs">{a.accountNo}</code>} />
                                <InfoRow icon={<Building2 className="h-4 w-4" />} label="Product" value={a.savingsProductName ?? a.savingsProductId} />
                                ...
                                <InfoRow
                                    ...
                                    label="Balance"
                                    value={<span className="font-semibold">{formatCurrency(summary.accountBalance, a.currency?.code)}</span>}
                                />
                                <InfoRow icon={<Percent className="h-4 w-4" />} label="Interest Rate" value={`${a.nominalAnnualInterestRate ?? 0}%`} />
                                <InfoRow
                        ...
                        <Card>
                            ...
                            <CardContent className="divide-y divide-gray-100 dark:divide-gray-800">
                                ...
                                />
                                <InfoRow icon={<Clock className="h-4 w-4" />} label="Last Transaction" value={fmtDate(a.lastActiveTransactionDate)} />
                            </CardContent>
                        </Card>
                        ...
                        <Card>
                            ...
                            <CardContent className="divide-y divide-gray-100 dark:divide-gray-800">
                                <InfoRow icon={<Calendar className="h-4 w-4" />} label="Submitted" value={fmtDate(a.timeline?.submittedOnDate)} />
                                <InfoRow icon={<Calendar className="h-4 w-4" />} label="Approved" value={fmtDate(a.timeline?.approvedOnDate)} />
                                <InfoRow icon={<Calendar className="h-4 w-4" />} label="Activated" value={fmtDate(a.timeline?.activatedOnDate)} />
                            </CardContent>
                        </Card>
                        ...
                        <Card className="col-span-full">
                            ...
                            <CardContent className="grid grid-cols-2 gap-4 text-sm">
                                <div>
                                    <span className="text-gray-500">Interest Rate:</span> <span className="font-medium">{a.nominalAnnualInterestRate ?? 0}%</span>
                                </div>
                                <div>
                                    <span className="text-gray-500">Compounding:</span> <span className="font-medium">{a.interestCompoundingPeriodType?.value ?? "—"}</span>
                                </div>
                                <div>
                                    <span className="text-gray-500">Posting:</span> <span className="font-medium">{a.interestPostingPeriodType?.value ?? "—"}</span>
                                </div>
                                <div>
                                    <span className="text-gray-500">Calculation:</span> <span className="font-medium">{a.interestCalculationType?.value ?? "—"}</span>
                                </div>
                                <div>
                                    <span className="text-gray-500">Days/Year:</span> <span className="font-medium">{a.interestCalculationDaysInYearType?.value ?? "—"}</span>
                                </div>
                                <div>
                                    <span className="text-gray-500">Min Opening Balance:</span>{" "}
                                    <span className="font-medium">{a.minRequiredOpeningBalance != null ? formatCurrency(a.minRequiredOpeningBalance, a.currency?.code) : "—"}</span>
                                </div>
                                <div>
                                ...
                                <div>
                                    <span className="text-gray-500">Allow Overdraft:</span> <span className="font-medium">{a.allowOverdraft ? "Yes" : "No"}</span>
                                </div>
                                <div>
                                    <span className="text-gray-500">Withhold Tax:</span> <span className="font-medium">{a.withHoldTax ? "Yes" : "No"}</span>
                                </div>
                            </CardContent>
                        </Card>
                    </div>
            ...
            {txnDialog && <DepositWithdrawDialog accountId={a.id} type={txnDialog} open={!!txnDialog} onOpenChange={() => setTxnDialog(null)} onSuccess={() => refetch()} />}
        </div>
    );
};
