...
const isCredit = (t: TransactionType): boolean => ["cash_deposit", "cheque_deposit", "transfer_in", "interest_credit"].includes(t);
...
const DepositTransactionsPage: React.FC = () => {
    ...
    const filtered = useMemo(() => {
        ...
        if (q)
            result = result.filter(
                (t) => t.transactionId.toLowerCase().includes(q) || t.description.toLowerCase().includes(q) || t.accountNumber.includes(q),
            );
    ...
    const columns: ColumnDef<DepositTransaction>[] = [
            ...
            cell: (r) => {
                ...
                return (
                    <div className="flex items-center gap-1.5">
                        ...
                        <Badge variant={credit ? "success" : "error"} size="sm">
                            {TXN_LABELS[r.type]}
                        </Badge>
                    </div>
                );
        ...
        },
        {
            ...
            cell: (r) => (
                <span
                    className={`font-mono text-sm font-semibold ${isCredit(r.type) ? "text-emerald-700 dark:text-emerald-400" : "text-red-700 dark:text-red-400"}`}
                >
                    {isCredit(r.type) ? "+" : "−"}
        ...
        },
        {
            key: "balanceBefore",
            header: "Before",
            cell: (r) => <span className="font-mono text-xs text-gray-500">{formatCurrency(r.balanceBefore)}</span>,
        },
        { key: "balanceAfter", header: "After", cell: (r) => <span className="font-mono text-sm">{formatCurrency(r.balanceAfter)}</span> },
        {
            key: "description",
            header: "Description",
            cell: (r) => <span className="text-xs max-w-45 truncate inline-block">{r.description}</span>,
        },
        {
            key: "channel",
            header: "Channel",
        ...
        { key: "status", header: "Status", cell: (r) => <StatusBadge status={r.status} size="sm" /> },
        {
            ...
            header: "Date",
            cell: (r) =>
                new Date(r.transactionDate).toLocaleDateString("en-US", {
                    month: "short",
                    day: "numeric",
                    hour: "2-digit",
                    minute: "2-digit",
    ...
    return (
        <div className="space-y-6">
            <PageHeader
                ...
                description="View all deposit and withdrawal transactions across accounts"
                actions={
