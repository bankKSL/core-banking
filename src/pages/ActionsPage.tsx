...
const actionColorMapSpec: Record<ActionType, { bg: string; text: string; label: string }> = {
    set_interest_rate: { bg: "bg-blue-100 dark:bg-blue-900/30", text: "text-blue-800 dark:text-blue-400", label: "Set Interest Rate" },
    apply_cashback: {
        bg: "bg-emerald-100 dark:bg-emerald-900/30",
        text: "text-emerald-800 dark:text-emerald-400",
        label: "Apply Cashback",
    },
    waive_fee: { bg: "bg-amber-100 dark:bg-amber-900/30", text: "text-amber-800 dark:text-amber-400", label: "Waive Fee" },
    add_reward_points: {
        bg: "bg-purple-100 dark:bg-purple-900/30",
        text: "text-purple-800 dark:text-purple-400",
        label: "Add Reward Points",
    },
    apply_penalty: { bg: "bg-red-100 dark:bg-red-900/30", text: "text-red-800 dark:text-red-400", label: "Apply Penalty" },
    adjust_limit: { bg: "bg-indigo-100 dark:bg-indigo-900/30", text: "text-indigo-800 dark:text-indigo-400", label: "Adjust Limit" },
};
    ...
    const actionColumns: ColumnDef<FlatAction>[] = [
        ...
        },
        {
            ...
            cell: (row) => {
                ...
                return (
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${spec.bg} ${spec.text}`}>
                        {spec.label}
                    </span>
                );
            },
        ...
        },
        {
            ...
            header: "Value",
            cell: (row) => (
                <code className="text-xs bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded font-mono font-semibold">{row.value}</code>
            ),
        },
        {
            key: "description",
            header: "Description",
            cell: (row) => <span className="text-sm text-gray-600 dark:text-gray-400 max-w-75 truncate block">{row.description}</span>,
        },
        {
            key: "campaignStatus",
