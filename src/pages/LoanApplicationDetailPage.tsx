...
const LoanApplicationDetailPage: React.FC = () => {
  ...
  if (isLoading) return (<div className="p-6 max-w-3xl m-auto space-y-6"><Skeleton className="h-10 w-64" /><div className="grid grid-cols-1 gap-6 lg:grid-cols-2">{[1,2,3,4].map(i=>(<Skeleton key={i} className="h-40 rounded-xl" />))}</div></div>);
  if (isError || !loan) return (<div className="flex items-center justify-center h-64"><div className="text-center"><AlertTriangle className="mx-auto h-8 w-8 text-red-500 mb-2" /><p className="text-red-600">Failed to load: {String(error)}</p><Button variant="outline" className="mt-2" onClick={() => refetch()}>Retry</Button></div></div>);
  ...
  return (
    <div className="p-6 max-w-4xl m-auto space-y-6">
      <PageHeader title={`Loan ${loan.accountNo ?? `#${loan.id}`}`} description={`${loan.loanProductName} — ${loan.clientName ?? `Client #${loan.clientId}`}`}
        actions={
          <div className="flex items-center gap-2">
            <Badge variant={statusCfg?.variant === "success" ? "success" : statusCfg?.variant === "error" ? "error" : "default"}>{statusCfg?.label ?? loan.status?.code}</Badge>
            {loan.status?.code === "loanStatusType.approved" && (
              <Button size="sm" onClick={() => disburseMut.mutate({ loanId: loan.id, payload: { locale: "en", dateFormat: "dd MMMM yyyy", actualDisbursementDate: new Date().toISOString().split("T")[0] } })} disabled={disburseMut.isPending}>
                {disburseMut.isPending ? "Disbursing…" : "Disburse"}
              </Button>
            )}
            {loan.status?.code === "loanStatusType.submitted.and.pending.approval" && (
              <Button size="sm" onClick={() => approveMut.mutate({ loanId: loan.id, payload: { locale: "en", dateFormat: "dd MMMM yyyy", approvedOnDate: new Date().toISOString().split("T")[0] } })} disabled={approveMut.isPending}>
                {approveMut.isPending ? "Approving…" : "Approve"}
              </Button>
            )}
            <Button variant="outline" onClick={() => navigate("/lending/applications")}><ArrowLeft className="mr-2 h-4 w-4" />Back</Button>
          </div>
        }
      />
      <Separator />
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><Landmark className="h-4 w-4 text-blue-500" />Loan Details</CardTitle></CardHeader>
          <CardContent className="divide-y divide-gray-100">
            ...
            <InfoRow icon={<Calendar className="h-4 w-4" />} label="Applied" value={loan.timeline?.submittedOnDate ? new Date(loan.timeline.submittedOnDate).toLocaleDateString() : "—"} />
          </CardContent>
        </Card>
        {summary && (
          <Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><DollarSign className="h-4 w-4 text-emerald-500" />Summary</CardTitle></CardHeader>
            <CardContent className="divide-y divide-gray-100">
        ...
        <Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><Calendar className="h-4 w-4 text-gray-400" />Schedule Preview</CardTitle></CardHeader>
          <CardContent>
            {schedule.length === 0 ? <p className="text-sm text-gray-400">No schedule available</p> : (
              <div className="space-y-1 max-h-72 overflow-y-auto">
                {schedule.slice(0, 12).map((p, i) => (
                  <div key={i} className="flex justify-between text-xs py-1.5 border-b last:border-0">
                    <span className="text-gray-500">#{p.period} {new Date(p.dueDate).toLocaleDateString("en-US", {month:"short",day:"numeric"})}</span>
