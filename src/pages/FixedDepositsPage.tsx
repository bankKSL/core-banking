...
const FixedDepositsPage: React.FC = () => {
    ...
    const { data: fdData, isLoading: fdLoading, isError: fdError } = useFixedDepositAccounts({ limit: PAGE_SIZE, offset: (page - 1) * PAGE_SIZE });
    ...
    const columns: ColumnDef<FixedDepositAccount>[] = [
        ...
        { key: "maturityDate", header: "Matures", cell: (r) => (r.maturityDate ? new Date(r.maturityDate).toLocaleDateString() : "—") },
        {
            ...
            header: "Status",
            cell: (r) => {
                const c = FIXED_DEPOSIT_STATUS_CONFIG[r.status?.code ?? ""] ?? { label: r.status?.value ?? "Unknown", variant: "outline" as const };
