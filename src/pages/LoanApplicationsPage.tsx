...
import { useLoans, useApproveLoan, useDisburseLoan, useRejectLoan, useCloseLoan, LOAN_STATUS_CONFIG } from "@/features/loans";
...
const LoanApplicationsPage: React.FC = () => {
    ...
    const filtered = useMemo(() => {
        ...
        if (q)
            result = result.filter(
                (l) => (l.clientName ?? "").toLowerCase().includes(q) || (l.accountNo ?? "").toLowerCase().includes(q) || l.loanProductName.toLowerCase().includes(q),
            );
    ...
    const confirmStatusChange = () => {
        ...
        const loanId = app.id;
        const basePayload = { locale: "en", dateFormat: "yyyy-MM-dd" };
        switch (newCommand) {
            case "approve":
    ...
    const columns: ColumnDef<Loan>[] = [
        ...
        { key: "annualInterestRate", header: "Rate", cell: (r) => `${r.annualInterestRate ?? 0}%` },
        {
            ...
            header: "Status",
            cell: (r) => {
                const c = LOAN_STATUS_CONFIG[r.status?.code ?? ""];
        ...
        { key: "loanOfficerName", header: "Officer", cell: (r) => <span className="text-xs">{r.loanOfficerName ?? "—"}</span> },
        {
            key: "timeline",
            header: "Submitted",
            cell: (r) => <span className="text-xs">{r.timeline?.submittedOnDate ? new Date(r.timeline.submittedOnDate).toLocaleDateString() : "—"}</span>,
        },
        {
            key: "actions",
            header: "",
            cell: (r) => (
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-44">
                        <DropdownMenuItem onClick={() => navigate(`/lending/applications/${r.id}`)}>
                            <Eye className="mr-2 h-4 w-4" /> View Details
                        </DropdownMenuItem>
                        {r.status?.code === "loanStatusType.submitted.and.pending.approval" && (
                            <>
                                <DropdownMenuItem onClick={() => handleStatusChange(r, "approve")}>
                                    <CheckCircle className="mr-2 h-4 w-4 text-emerald-600" /> Approve
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => handleStatusChange(r, "reject")}>
                                    <XCircle className="mr-2 h-4 w-4 text-red-600" /> Reject
                                </DropdownMenuItem>
                            </>
                        )}
                        {r.status?.code === "loanStatusType.approved" && (
                            <DropdownMenuItem onClick={() => handleStatusChange(r, "disburse")}>
                                <DollarSign className="mr-2 h-4 w-4 text-blue-600" /> Disburse
                            </DropdownMenuItem>
                        )}
                        {r.status?.code === "loanStatusType.active" && (
                            <DropdownMenuItem onClick={() => handleStatusChange(r, "close")}>
                                <XCircle className="mr-2 h-4 w-4 text-gray-600" /> Close
                            </DropdownMenuItem>
                        )}
                    </DropdownMenuContent>
                </DropdownMenu>
        ...
        return (
            <div className="flex items-center justify-center h-64">
                <div className="text-center">
                    ...
                    <p className="text-red-600">Failed to load loans: {String(error)}</p>
            ...
            <Dialog open={!!statusDialog} onOpenChange={() => setStatusDialog(null)}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Update {statusDialog?.app.clientName ?? `Loan #${statusDialog?.app.id}`} to <strong>{statusDialog?.newCommand}</strong>?
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
                        ...
                        <Button
                            ...
                            disabled={approveMutation.isPending || disburseMutation.isPending || rejectMutation.isPending || closeMutation.isPending}
                        >
                            {approveMutation.isPending || disburseMutation.isPending || rejectMutation.isPending || closeMutation.isPending ? "Processing…" : "Confirm"}
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
