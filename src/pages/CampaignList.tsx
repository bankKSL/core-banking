...
import { Plus, Search, Download, Filter, MoreHorizontal, Eye, Pencil, Copy, Trash2, Power, PowerOff } from "lucide-react";
...
const CampaignList: React.FC = () => {
    ...
    const filtered = useMemo(() => {
        ...
        return campaigns.filter(
            (c) => c.name.toLowerCase().includes(q) || c.categoryName.toLowerCase().includes(q) || c.createdBy.toLowerCase().includes(q),
        );
    }, [search]);
    ...
    const getProductNames = useCallback((ids: string[]) => ids.map((id) => products.find((p) => p.id === id)?.name ?? id).join(", "), []);
    ...
    const columns: ColumnDef<Campaign>[] = [
        ...
        { key: "products", header: "Products", cell: (row) => <span className="text-xs">{getProductNames(row.products)}</span> },
        {
            key: "priority",
            header: "Priority",
            cell: (row) => (
