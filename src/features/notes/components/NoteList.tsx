...
const NoteList: FC<NoteListProps> = ({ resourceType, resourceId }) => {
  const { data: notes, isLoading } = useNotes(resourceType, resourceId);
  ...
  const onSubmit = useCallback(
    async (values: NoteFormValues) => {
      if (editingId) {
        await updateMutation.mutateAsync({ resourceType, resourceId, noteId: editingId, payload: values });
      } else {
        await createMutation.mutateAsync({ resourceType, resourceId, payload: values });
      }
      setDialogOpen(false);
    },
    [resourceType, resourceId, editingId, createMutation, updateMutation],
  );

  const handleDelete = useCallback(async () => {
    if (!deleteId) return;
    await deleteMutation.mutateAsync({ resourceType, resourceId, noteId: deleteId });
    setDeleteId(null);
  }, [resourceType, resourceId, deleteId, deleteMutation]);
    ...
    <div className="space-y-4">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      {formatDate(note.createdOn)} {note.createdByUsername ? `by ${note.createdByUsername}` : ""}
                    </p>
                  </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Enter note content.</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={!!viewingNote} onOpenChange={() => setViewingNote(null)}>
        <DialogContent>
          ...
          {viewingNote?.createdOn && <p className="text-xs text-gray-400">{formatDate(viewingNote.createdOn)}</p>}
        </DialogContent>
      </Dialog>
