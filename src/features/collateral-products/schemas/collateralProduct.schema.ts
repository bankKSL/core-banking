...
export const createCollateralProductSchema = z.object({
  ...
  pctToBase: z.number({ message: "Pct to base is required" }).positive("Must be positive").max(100, "Max 100%"),
...
export const updateCollateralProductSchema = z.object({
  ...
  pctToBase: z.number({ message: "Pct to base is required" }).positive("Must be positive").max(100, "Max 100%").optional(),
