    ...
    },
    {
      ...
      cell: (r) => (
        <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
          <Button variant="ghost" size="sm" onClick={() => navigate(`/admin/roles/${r.id}`)}>
            <Eye className="h-4 w-4" />
          </Button>
          <Button variant="ghost" size="sm" onClick={() => navigate(`/admin/roles/edit/${r.id}`)}>
            <Pencil className="h-4 w-4" />
          </Button>
          {r.disabled ? (
            <Button variant="ghost" size="sm" onClick={() => enableMutation.mutate(r.id)}>
              <ToggleRight className="h-4 w-4 text-green-500" />
            </Button>
          ) : (
            <Button variant="ghost" size="sm" onClick={() => disableMutation.mutate(r.id)}>
