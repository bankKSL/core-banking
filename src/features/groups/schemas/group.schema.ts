 ...
 * rule: when `active` is true, `activationDate` is mandatory.
 */
export const createGroupSchema = z
  .object({
    name: z
      .string({ message: i18n.t("Group name is required") })
      .min(1, i18n.t("Group name is required"))
      .max(100),
    officeId: z
      .number({ message: i18n.t("Office is required") })
      .int()
      .positive(i18n.t("Office is required")),
...
export const activateGroupSchema = z.object({
  activationDate: z
    .string({ message: i18n.t("Activation date is required") })
    .min(1, i18n.t("Activation date is required")),
});
