...
const PeriodicAccrualPage: React.FC = () => {
  ...
  return (
    <div className="max-w-6xl m-auto space-y-6">
      <PageHeader
        title={t("Periodic Accrual Accounting")}
        description={t("Accrue loan income up to a specified date for all active loans.")}
      />

      <Card>
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            <CalendarClock className="h-4 w-4" /> {t("Execute Accrual")}
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-1.5">
            <label className="block text-sm font-medium">{t("Accrue Till Date")} *</label>
            <Input type="date" value={tillDate} onChange={(e) => setTillDate(e.target.value)} />
            <p className="text-xs text-gray-500">
              {t("Accrual entries will be created for all active loans up to and including this date.")}
            </p>
          </div>
          ...
          {success && (
            <div className="flex items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-700 dark:border-emerald-800 dark:bg-emerald-950 dark:text-emerald-400">
              ...
              {t("Accrual executed successfully.")}
            </div>
          )}
          <div className="flex justify-end">
            ...
            >
              {executeMutation.isPending ? (
                <>
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" /> {t("Executing…")}
                </>
              ) : (
                <>
                  <Play className="mr-2 h-4 w-4" /> {t("Execute Accrual")}
                </>
              )}
            </Button>
          </div>
