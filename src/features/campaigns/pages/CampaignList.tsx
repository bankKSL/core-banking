...
const CampaignList: React.FC = () => {
  ...
  const columns: ColumnDef<Campaign>[] = [
    ...
    { key: "startDate", header: t("Start Date"), cell: (row) => new Date(row.startDate).toLocaleDateString() },
    { key: "endDate", header: t("End Date"), cell: (row) => new Date(row.endDate).toLocaleDateString() },
    { key: "version", header: t("Version"), cell: (row) => <span className="text-xs font-mono">v{row.version}</span> },
    { key: "createdBy", header: t("Created By") },
    {
      key: "actions",
      header: t("Actions"),
      cell: (row) => (
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={t("Manage banking campaigns and promotional rules")}
        actions={
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard title={t("Total Campaigns")} value={stats.total} />
        <StatCard title={t("Active")} value={stats.active} variant="success" />
        <StatCard title={t("Scheduled")} value={stats.scheduled} variant="warning" />
        <StatCard title={t("Expired")} value={stats.expired} variant="error" />
      </div>
      <Card>
