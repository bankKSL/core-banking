      ...
      switch (cmd) {
        case "approve":
          await approveMut.mutateAsync({
            loanId: loan.id,
            payload: { approvedOnDate: dateInput, dateFormat: "yyyy-MM-dd", locale: "en" },
          });
          setDateDialog(false);
          break;
        case "disburse":
        ...
        case "close":
          await closeMut.mutateAsync({
            loanId: loan.id,
            payload: { closedOnDate: dateInput, dateFormat: "yyyy-MM-dd", locale: "en" },
          });
          setDateDialog(false);
          break;
        case "undoApproval":
  ...
  return (
    <>
      <div className="flex flex-wrap items-center gap-2">
        ...
        {isPending && (
          <>
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => setCommand("withdrawnByClient")}
        ...
        {isApproved && (
          <>
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => setCommand("undoApproval")}
              className="text-amber-600 border-amber-200 hover:bg-amber-50"
            >
        ...
        {isActive && (
          <>
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => navigate(`/loans/${loan.id}/transactions/waiveinterest`)}
            >
              <Ban className="mr-1 h-4 w-4" />
              Waive Interest
            </Button>
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => navigate(`/loans/${loan.id}/transactions/writeoff`)}
              className="text-red-600 border-red-200 hover:bg-red-50"
            >
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => navigate(`/loans/${loan.id}/transactions/foreclosure`)}
              className="text-red-600 border-red-200 hover:bg-red-50"
            >
              <Ban className="mr-1 h-4 w-4" />
              Foreclose
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => setCommand("undoDisbursal")}
              className="text-amber-600 border-amber-200 hover:bg-amber-50"
            >
        ...
        {(isClosed || isOverpaid) && <span className="text-sm text-gray-400 italic">No actions available</span>}
      </div>
      ...
      <Dialog open={dateDialog} onOpenChange={setDateDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {command === "approve" ? "Approve Loan" : command === "disburse" ? "Disburse Loan" : "Close Loan"}
            </DialogTitle>
            <DialogDescription>
              Enter the {command === "approve" ? "approval" : command === "disburse" ? "disbursement" : "closure"} date.
            </DialogDescription>
          </DialogHeader>
      ...
      {command === "withdrawnByClient" && (
        <ConfirmDialog
          ...
          onConfirm={() => handleCommand("withdrawnByClient")}
          variant="destructive"
      ...
      {command === "undoDisbursal" && (
        <ConfirmDialog
          ...
          onConfirm={() => handleCommand("undoDisbursal")}
          variant="destructive"
          confirmLabel="Undo"
          loading={isMutating}
        />
