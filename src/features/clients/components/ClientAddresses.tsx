...
import { useClientAddresses, useClientAddressTemplate, useCreateClientAddress, useUpdateClientAddress, useDeleteClientAddress } from "../hooks/useClientAddresses";
...
interface ClientAddressesProps { clientId: number }
...
const ClientAddresses: FC<ClientAddressesProps> = ({ clientId }) => {
    ...
    const openEdit = useCallback((addr: ClientAddress) => {
    ...
    const columns: ColumnDef<ClientAddress>[] = [
        { key: "addressType", header: "Type", accessorFn: (row) => <span className="text-sm font-medium">{row.addressType ?? "—"}</span> },
        { key: "addressLine1", header: "Address", accessorFn: (row) => <span className="text-sm">{row.addressLine1 ?? row.street ?? "—"}{row.city ? `, ${row.city}` : ""}</span> },
        { key: "postalCode", header: "Postal Code", accessorFn: (row) => <span className="text-sm">{row.postalCode ?? "—"}</span> },
        { key: "isActive", header: "Active", accessorFn: (row) => row.isActive ? <Badge variant="success" size="sm">Active</Badge> : <Badge variant="default" size="sm">Inactive</Badge> },
        { key: "actions", header: "Actions", accessorFn: (row) => (
            <div className="flex items-center gap-1">
                ...
                <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); setDeleteId(row.addressId); }}><Trash2 className="h-4 w-4 text-red-500" /></Button>
            </div>
        )},
    ];
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <h3 className="text-lg font-medium flex items-center gap-2"><MapPin className="h-5 w-5" />Addresses</h3>
                <Button onClick={openCreate} size="sm"><Plus className="mr-1 h-4 w-4" />Add Address</Button>
            </div>
            <Card><CardContent className="p-0">
                <DataTable columns={columns} data={addresses ?? []} loading={isLoading} minWidth={600}
                    emptyState={{ icon: <MapPin className="h-8 w-8 text-gray-300" />, message: "No addresses found." }} />
            </CardContent></Card>

            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                <DialogContent className="sm:max-w-lg">
                    <DialogHeader><DialogTitle>{editingAddressId ? "Edit Address" : "Add Address"}</DialogTitle><DialogDescription>Enter address details.</DialogDescription></DialogHeader>
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="flex flex-col gap-1.5">
                                <Label>Address Type</Label>
                                <Select defaultValue={String(template?.addressTypeIdOptions?.[0]?.id ?? "")}
                                    onValueChange={(v) => setValue("addressTypeId", Number(v))}>
                                    <SelectTrigger><SelectValue placeholder="Type" /></SelectTrigger>
                                    <SelectContent>{template?.addressTypeIdOptions?.map((o) => <SelectItem key={o.id} value={String(o.id)}>{o.name}</SelectItem>)}</SelectContent>
                                </Select>
                            </div>
                            <div className="flex flex-col gap-1.5">
                        ...
                        <div className="grid grid-cols-3 gap-4">
                            ...
                            <div className="flex flex-col gap-1.5">
                                ...
                                <Select onValueChange={(v) => setValue("stateProvinceId", Number(v))}>
                                    ...
                                    <SelectContent>{template?.stateProvinceIdOptions?.map((o) => <SelectItem key={o.id} value={String(o.id)}>{o.name}</SelectItem>)}</SelectContent>
                                </Select>
                            </div>
                            <div className="flex flex-col gap-1.5">
                                <Label>Country</Label>
                                <Select onValueChange={(v) => setValue("countryId", Number(v))}>
                                    <SelectTrigger><SelectValue placeholder="Select" /></SelectTrigger>
                                    <SelectContent>{template?.countryIdOptions?.map((o) => <SelectItem key={o.id} value={String(o.id)}>{o.name}</SelectItem>)}</SelectContent>
                                </Select>
                            </div>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
            ...
            <ConfirmDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)} title="Delete Address"
