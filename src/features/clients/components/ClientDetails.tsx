...
const ClientDetails: FC<ClientDetailsProps> = ({ client }) => {
    ...
    return (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <Card>
                <CardHeader><CardTitle className="flex items-center gap-2 text-base"><Hash className="h-4 w-4 text-gray-400" />Basic Information</CardTitle></CardHeader>
            ...
            <Card>
                <CardHeader><CardTitle className="flex items-center gap-2 text-base"><User className="h-4 w-4 text-gray-400" />Personal Information</CardTitle></CardHeader>
                <CardContent className="divide-y divide-gray-100 dark:divide-gray-800">
                    ...
                    <InfoRow icon={<Calendar className="h-4 w-4" />} label="Date of Birth"
                        value={client.dateOfBirth ? `${formatClientDate(client.dateOfBirth)}${age !== null ? ` (${age} years)` : ""}` : "—"} />
                </CardContent>
            </Card>

            <Card>
                <CardHeader><CardTitle className="flex items-center gap-2 text-base"><Phone className="h-4 w-4 text-gray-400" />Contact Information</CardTitle></CardHeader>
                <CardContent className="divide-y divide-gray-100 dark:divide-gray-800">
            ...
            <Card>
                <CardHeader><CardTitle className="flex items-center gap-2 text-base"><Clock className="h-4 w-4 text-gray-400" />Timeline</CardTitle></CardHeader>
                <CardContent className="divide-y divide-gray-100 dark:divide-gray-800">
                    <InfoRow icon={<Calendar className="h-4 w-4" />} label="Submitted On" value={formatClientDate(client.timeline?.submittedOnDate)} />
                    <InfoRow icon={<Calendar className="h-4 w-4" />} label="Activated On" value={formatClientDate(client.activationDate ?? client.timeline?.activatedOnDate)} />
                    <InfoRow icon={<Calendar className="h-4 w-4" />} label="Closed On" value={formatClientDate(client.timeline?.closedOnDate)} />
                </CardContent>
            </Card>
        </div>
    );
