...
import { useClientIdentifiers, useClientIdentifierTemplate, useCreateClientIdentifier, useUpdateClientIdentifier, useDeleteClientIdentifier } from "../hooks/useClientIdentifiers";
...
const ClientIdentifiers: FC<ClientIdentifiersProps> = ({ clientId }) => {
    ...
    const { register, handleSubmit, reset, setValue, formState: { errors } } = useForm<IdentifierFormValues>({
    ...
    const openEdit = useCallback((id: ClientIdentifier) => {
        setEditingId(id.id);
        reset({ documentTypeId: id.documentType.id, documentKey: id.documentKey, description: id.description ?? "" });
        setDialogOpen(true);
    }, [reset]);

    const onSubmit = useCallback(async (values: IdentifierFormValues) => {
        if (editingId) {
            await updateMutation.mutateAsync({ clientId, identifierId: editingId, payload: values });
        } else {
            await createMutation.mutateAsync({ clientId, payload: { ...values, documentTypeId: Number(values.documentTypeId) } });
        }
        setDialogOpen(false);
    ...
    const columns: ColumnDef<ClientIdentifier>[] = [
        { key: "documentType", header: "Document Type", accessorFn: (row) => <span className="text-sm font-medium">{row.documentType?.name ?? "—"}</span> },
        { key: "documentKey", header: "Document Key", accessorFn: (row) => <span className="text-sm font-mono">{row.documentKey}</span> },
        { key: "description", header: "Description", accessorFn: (row) => <span className="text-sm text-gray-500">{row.description ?? "—"}</span> },
        { key: "status", header: "Status", accessorFn: (row) => row.status ? <Badge variant={row.status === "active" ? "success" : "default"} size="sm">{row.status}</Badge> : <span className="text-sm text-gray-400">—</span> },
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <h3 className="text-lg font-medium flex items-center gap-2"><Fingerprint className="h-5 w-5" />Identifiers</h3>
                <Button onClick={openCreate} size="sm"><Plus className="mr-1 h-4 w-4" />Add Identifier</Button>
            </div>
            <Card>
                <CardContent className="p-0">
                    <DataTable columns={columns} data={identifiers ?? []} loading={isLoading} minWidth={600}
                        emptyState={{ icon: <Fingerprint className="h-8 w-8 text-gray-300" />, message: "No identifiers found. Click 'Add Identifier' to create one." }} />
                </CardContent>
            </Card>
                ...
                <DialogContent>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                        <div className="flex flex-col gap-1.5">
                            ...
                            <Select defaultValue={String(template?.allowedDocumentTypes?.[0]?.id ?? "")}
            ...
            <ConfirmDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)} title="Delete Identifier"
