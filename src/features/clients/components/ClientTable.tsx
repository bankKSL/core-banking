...
const ClientTable: FC<ClientTableProps> = ({ data, loading, onRowClick }) => {
    ...
    const columns: ColumnDef<Client>[] = [
        ...
        },
        {
            ...
            header: "Account No",
            accessorFn: (row) => (
                <span className="text-sm">{row.accountNo ?? "—"}</span>
            ),
            sortable: true,
        },
        {
            ...
            header: "External ID",
            accessorFn: (row) => (
                <span className="text-sm text-gray-500">{row.externalId ?? "—"}</span>
            ),
            sortable: true,
        },
        {
        ...
        },
        {
            ...
            header: "Office",
            accessorFn: (row) => (
                <span className="text-sm">{row.officeName ?? "—"}</span>
            ),
            sortable: true,
        },
        {
        ...
        },
        {
            ...
            header: "Mobile",
            accessorFn: (row) => (
                <span className="text-sm">{row.mobileNo ?? "—"}</span>
            ),
        },
        {
            key: "status",
            header: "Status",
            accessorFn: (row) => (
                <ClientStatusBadge status={getClientStatus(row)} size="sm" />
            ),
            sortable: true,
        },
        {
            ...
            accessorFn: (row) => (
                <span className="text-sm text-gray-600 dark:text-gray-400">
                    {formatClientDate(row.activationDate)}
                </span>
            ),
            sortable: true,
        },
