 ...
 */
export async function fetchClientFamilyMembers(clientId: number | string): Promise<ClientFamilyMember[]> {
  const { data } = await client.get<ClientFamilyMember[]>(`/clients/${clientId}/familymembers`);
  return data;
}
 ...
 */
export async function fetchClientFamilyMemberTemplate(clientId: number | string): Promise<ClientFamilyMemberTemplate> {
  const { data } = await client.get<ClientFamilyMemberTemplate>(`/clients/${clientId}/familymembers/template`);
  return data;
}
...
export async function fetchClientFamilyMember(
  ...
  const { data } = await client.get<ClientFamilyMember>(`/clients/${clientId}/familymembers/${familyMemberId}`);
  return data;
}
...
export async function createClientFamilyMember(
  ...
  const { data } = await client.post<ClientFamilyMemberCommandResponse>(`/clients/${clientId}/familymembers`, payload);
  return data;
}
