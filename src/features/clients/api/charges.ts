...
export async function fetchClientCharges(
  ...
  const { data } = await client.get<ClientChargeListResponse>(`/clients/${clientId}/charges`, { params });
  return data;
}
 ...
 */
export async function fetchClientChargesTemplate(clientId: number | string): Promise<ClientChargesTemplate> {
  const { data } = await client.get<ClientChargesTemplate>(`/clients/${clientId}/charges/template`);
  return data;
}
...
export async function createClientCharge(
  ...
  const { data } = await client.post<ClientChargeCommandResponse>(`/clients/${clientId}/charges`, payload);
  return data;
}
...
export async function deleteClientCharge(
  ...
  const { data } = await client.delete<ClientChargeCommandResponse>(`/clients/${clientId}/charges/${chargeId}`);
  return data;
}
