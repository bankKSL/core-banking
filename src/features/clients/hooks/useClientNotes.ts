...
export const clientNoteKeys = {
    ...
    detail: (clientId: number | string, noteId: number | string) => [...clientKeys.detail(clientId), "notes", noteId] as const,
};
...
export function useUpdateClientNote() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ clientId, noteId, payload }: { clientId: number | string; noteId: number | string; payload: ClientNoteRequest }) =>
