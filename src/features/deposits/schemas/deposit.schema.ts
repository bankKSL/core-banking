...
export const createRecurringDepositProductSchema = z.object({
  ...
  inMultiplesOf: z.number().int().min(0).optional(),
  depositAmount: z
    .number({ message: i18n.t("Deposit amount is required") })
    .min(0, i18n.t("Deposit amount must be >= 0")),
  minDepositTerm: z
    .number({ message: i18n.t("Min term is required") })
    .int()
    .positive(i18n.t("Min term must be > 0")),
  ...
  recurringDepositFrequency: z
    .number({ message: i18n.t("Frequency is required") })
    .int()
    .positive(i18n.t("Frequency must be > 0")),
