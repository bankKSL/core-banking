...
import { fetchSavingsCharges, fetchSavingsChargesTemplate, createSavingsCharge, waiveSavingsCharge, deleteSavingsCharge } from "../api/deposit";
...
export function useCreateSavingsCharge() {
    ...
    return useMutation({
        mutationFn: ({ accountId, payload }: { accountId: number | string; payload: PostSavingsChargeRequest }) => createSavingsCharge(accountId, payload),
        onSuccess: (_data, variables) => { qc.invalidateQueries({ queryKey: savingsChargeKeys.all(variables.accountId) }); },
    });
}
...
export function useWaiveSavingsCharge() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: ({ accountId, chargeId }: { accountId: number | string; chargeId: number | string }) => waiveSavingsCharge(accountId, chargeId),
        onSuccess: (_data, variables) => { qc.invalidateQueries({ queryKey: savingsChargeKeys.all(variables.accountId) }); },
    });
}
...
export function useDeleteSavingsCharge() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: ({ accountId, chargeId }: { accountId: number | string; chargeId: number | string }) => deleteSavingsCharge(accountId, chargeId),
        onSuccess: (_data, variables) => { qc.invalidateQueries({ queryKey: savingsChargeKeys.all(variables.accountId) }); },
    });
}
