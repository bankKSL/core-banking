...
const DocumentList: FC<DocumentListProps> = ({ entityType, entityId }) => {
  ...
  const onSubmit = useCallback(
    async (values: DocFormValues) => {
      if (editingDoc) {
        await updateMutation.mutateAsync({
          entityType,
          entityId,
          documentId: editingDoc.id,
          payload: { ...values, file: selectedFile ?? undefined },
        });
      } else {
        if (!selectedFile) return;
        await createMutation.mutateAsync({ entityType, entityId, payload: { ...values, file: selectedFile } });
      }
      setDialogOpen(false);
    },
    [entityType, entityId, editingDoc, selectedFile, createMutation, updateMutation],
  );

  const handleDelete = useCallback(async () => {
    if (!deleteId) return;
    await deleteMutation.mutateAsync({ entityType, entityId, documentId: deleteId });
    setDeleteId(null);
  }, [entityType, entityId, deleteId, deleteMutation]);

  const handleDownload = useCallback(
    async (doc: Document) => {
      setDownloadingId(doc.id);
      try {
        const blob = await downloadDocument(entityType, entityId, doc.id);
